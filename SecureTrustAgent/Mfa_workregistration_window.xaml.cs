...
namespace SecureTrustAgent
{
    ...
    /// </summary>
    public partial class Mfa_workregistration_window : Window
    {
        int g_nCnt = 0;
        ictk_puf_warpper puf_obj;
        ICTK_PUF ictk_puf_obj;
        DispatcherTimer timer;
        public Mfa_workregistration_window(ictk_puf_warpper puf_Warpper, ICTK_PUF ictk_puf)
        {
            puf_obj = puf_Warpper;
            ictk_puf_obj = ictk_puf;
            InitializeComponent();
        ...
        public void OnEnrollEvent(int ret, int remainCnt)
        {
            int nMaxCnt = 5;
            regwin_subtitle.Text = Localization.res.STR_MFA_AUTHHELPSUBMESSAGE;

            if (remainCnt == nMaxCnt)
            {
                var messageBoxResult = CustomMessageBoxClass.Show(Localization.res.STR_SIGNUP_TITLE, Localization.res.STR_MFA_REGPUFHELPMSG, MessageBoxButton.OK);
                this.Close();
            }
        ...
        public async void puf_enroll_work()
        {
            bool result = await EnrollFP_Working();
            
            if (result == true) {

                result = ictk_puf_obj.macverify_fingerprintf_of_puf();
                if (result == true)
                {
                    byte [] bytes = ictk_puf_obj.g3berify_fingerprintf_of_puf();
                }
            ...
            //puf_obj.run_enroll_fingerprint();
        }

        public Task<bool> EnrollFP_Working()
        {
            return Task.Factory.StartNew(() => puf_obj.run_enroll_fingerprint());
